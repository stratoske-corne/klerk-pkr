...
The draft/commit review step (PKR_SPEC.md §7's actual full design), version
diffing (`pkr diff v0.1 v0.2` — the data is already there, just not
surfaced), and a real `pkr commit`/`pkr confirm`/`pkr edit` human-correction
workflow. None of these are abandoned — §7's spec text still describes the
target shape — they're scoped out of this slice for the reason above, not
overlooked.
